    ...
    if (isset($_REQUEST['submit'])) {
        ...
        try {
            ...
            if (
                ...
                !empty($province_current) and !empty($highschool) and !empty($season_hsc) and !empty($district_study) and !empty($province_study) and !empty($image_file)
            ) {
                ...
                // :guardian_tel, :village_birth, :district_birth, :province_birth, :village_current, :district_current, :province_current, :house_unit, :house_no, :season_start, :highschool, :season_hsc,
                // :village_study, :district_study, :province_study, :employment_history, :language_proficiency, :talent, :familymatters, :plansforthefuture, :image)');
                if ($study_program == 'Bachelor Degree') {

                    $path = "upload/student_profile/Bachelor_students/" . $image_file; // set upload folder path
                    move_uploaded_file($temp, 'upload/student_profile/Bachelor_students/' . $image_file); // move upload file temperory directory to your upload folder

                    $stmt2 = $conn->prepare('INSERT INTO bechelor_students(std_id, u_id, fname_en, lname_en, gender, fname_la, lname_la, program, fname_ch, lname_ch, dob, part, nation, religion, ethnicity, tel, whatsapp, email, 
                    guardian_tel, village_birth, district_birth, province_birth, village_current, district_current, province_current, house_unit, house_no, highschool, season_hsc,
                    district_study, province_study, employment_history, language_proficiency, talent, familymatters, plansforthefuture, image) 
                                        VALUES(:std_id, :u_id, :fname_en, :lname_en, :gender, :fname_la, :lname_la, :program, :fname_ch, :lname_ch, :dob, :part, :nation, :religion, :ethnicity, :tel, :whatsapp, :email,
                    :guardian_tel, :village_birth, :district_birth, :province_birth, :village_current, :district_current, :province_current, :house_unit, :house_no, :highschool, :season_hsc,
                    :district_study, :province_study, :employment_history, :language_proficiency, :talent, :familymatters, :plansforthefuture, :image)');
                    ...
                    exit;
                } else if ($study_program == 'Diploma Degree') {

                    $path = "upload/student_profile/Diploma_students/" . $image_file; // set upload folder path
                    move_uploaded_file($temp, 'upload/student_profile/Diploma_students/' . $image_file); // move upload file temperory directory to your upload folder

                    $stmt2 = $conn->prepare('INSERT INTO diploma_students(std_id, u_id, fname_en, lname_en, gender, fname_la, lname_la, program, fname_ch, lname_ch, dob, part, nation, religion, ethnicity, tel, whatsapp, email, 
                    guardian_tel, village_birth, district_birth, province_birth, village_current, district_current, province_current, house_unit, house_no, highschool, season_hsc,
                    district_study, province_study, employment_history, language_proficiency, talent, familymatters, plansforthefuture, image) 
                                        VALUES(:std_id, :u_id, :fname_en, :lname_en, :gender, :fname_la, :lname_la, :program, :fname_ch, :lname_ch, :dob, :part, :nation, :religion, :ethnicity, :tel, :whatsapp, :email,
                    :guardian_tel, :village_birth, :district_birth, :province_birth, :village_current, :district_current, :province_current, :house_unit, :house_no, :highschool, :season_hsc,
                    ...
                    // If there is an error, please check the program type in wabpage and database. make sure those are sames
                    $_SESSION['error'] = "Add Student Error, Please try again!";
                    header('location: student-add.php');
                    exit;
                }
            }
