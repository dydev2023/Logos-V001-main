...
$principal = $fname_en = $lname_en = $gender = $fname_la = $lname_la = $dob = $fname_ch = '';
...
$principal_err = $fname_en_err = $lname_en_err = $gender_err = $fname_la_err = $lname_la_err = $dob_err = $fname_ch_err = '';
...
$principal_red_border = $fname_en_red_border = $lname_en_red_border = $gender_red_border = $fname_la_red_border = $lname_la_red_border = $dob_red_border = $fname_ch_red_border = '';
...
if (!isset($_SESSION['admin_login'])) {
    ...
    $check_u_id->bindParam(":u_id", $_REQUEST['principal_id']);
    $check_u_id->execute();
    ...
    // Select principal data in Database For Check
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (empty($_REQUEST['principal'])) {
            $principal_err = 'Principal ID is required!';
            $principal_red_border = 'red_border';
        } elseif ($check_u_id->rowCount() > 0) {
            $principal_err = 'The Principal ID is already exsist!';
            $principal_red_border = 'red_border';
        ...
        if (empty($_REQUEST['tel'])) {
            ...
            $tel_err = 'The Tel is already exsist!';
            $tel_red_border = 'red_border';
            $tel = $_REQUEST['tel'];
        } else {
            $tel = $_REQUEST['tel'];
        ...
        if (empty($_REQUEST['whatsapp'])) {
            ...
            $whatsapp_err = 'The whatsapp is already exsist!';
            $whatsapp_red_border = 'red_border';
        ...
        if (empty($_REQUEST['email'])) {
            ...
            $email_err = 'The E-mail is already exist!';
            $email_red_border = 'red_border';
        ...
        if (empty($_FILES['txt_file']['name'])) {
            $image_file_err = "Principal image is required!";
        } else {
        ...
        if (
            !empty($principal) && !empty($fname_en) && !empty($lname_en) && !empty($gender) && !empty($fname_la) && !empty($th_type) && !empty($deploma) && !empty($doctorate) &&
            ...
            try {
                ...
                if ($type == "image/jpg" || $type == 'image/jpeg' || $type == "image/png" || $type == "image/gif") {
                    $passwordHash = password_hash($principal, PASSWORD_DEFAULT);

                    // Add User
                    $stmt1 = $conn->prepare('INSERT INTO users(u_id, u_email, u_pass, status) 
                                                                VALUES (:u_id, :u_email, :u_pass, :status)');
                    $stmt1->bindParam(':u_id', $principal);
                    $stmt1->bindParam(':u_email', $email);
                    $stmt1->bindParam(':u_pass', $passwordHash);
                    $stmt1->bindParam(':status', $status);

                    // Add Admin
                    $stmt2 = $conn->prepare('INSERT INTO teachers(id,principal, fname_en, lname_en, fname_la, lname_la, fname_ch, lname_ch, gender, dob, tel, whatsapp, email, village, district, province, nation, religion, university, image) 
                                        VALUES(LAST_INSERT_ID(), :principal, :fname_en, :lname_en, :fname_la, :lname_la, :fname_ch, :lname_ch, :gender, :dob, :tel, :whatsapp, :email, :village, :district, :province, :nation, :religion, :university, :image)');
                    $stmt2->bindParam(':principal', $principal);
                    ...
                    $path = "upload/principal_profile/" . $image_file; // set upload folder path
                    move_uploaded_file($temp, 'upload/principal_profile/' . $image_file); // move upload file temperory directory to your upload folder

                    $_SESSION['success'] = "Add principal successfully. <a href='logos-principal-list.php'> Click here to see the detail. </a>";
                    header('location: logos-principal-add.php');
                    exit;
                } else {
                    $_SESSION['error'] = "Upload JPG, JPEG & PNG file formate!";
                    header('location: principal-add.php');
                    exit;
                }
                    ...
                    <div class="row align-items-center">
                        <div class="col-sm-12">
                            <div class="page-sub-header">
                                <h3 class="page-title">Edit Principal</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="logos-principal-list.php">Principals</a></li>
                        ...
                        <div class="card comman-shadow">
                            <div class="card-body">
                                <form method="post" action="" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <h5 class="form-title student-info">Principal Information <span><a href="javascript:;"><i class="feather-more-vertical"></i></a></span></h5>
                                        </div>
                                        <div class="col-12 col-sm-4">
                                            <div class="form-group local-forms">
                                                <label>Principal ID <span class="login-danger">*</span> </label>
                                                <input class="form-control <?php echo $principal_red_border ?>" type="text" name="principal" value="<?php echo $principal ?>">
                                                <div class="error"><?php echo $principal_err ?></div>
                                            </div>
                                        </div>
                                        <div class="col-12 col-sm-4">
                                            <div class="form-group local-forms">
